...
const DEFAULTS = {
  ...
  // Нейронка переписывает ТОЛЬКО текстовые словари рассылки (MESSAGES/PASTE/
  ...
  ai: {
    ...
    // Куда идти за моделью: 'deepseek' | 'tooken' | свой адрес в baseUrl.
    // Адреса провайдеров - в ai/aiClient.js, здесь только выбор.
    provider: 'deepseek',
    baseUrl: '', // свой адрес, если провайдер не из списка (перебивает выбор)
    apiKey: '', // ключ провайдера, вводит пользователь
    model: 'deepseek-chat',
  ...
  parser: {
    ...
    countries: ['us'],
    // Фильтры парсинга деревом filters[типAPI][площадка] = { ключ: значение }.
    // Двухуровневый ключ обязателен: у XProject фильтр цены называется
    // min_price и он число, у VVS - price и это строка вида "10..100", а
    // категория Depop ничего не значит для Vinted. Что за ключи бывают и как
    // они приводятся - src/main/parser/filters.js.
    filters: {},
  },
  ...
  // Мастер первого запуска и подсказки показываются один раз. Флаги живут в
  // настройках, а не в памяти окна: иначе мастер встречал бы человека при
  // каждом запуске, а он нужен ровно однажды.
  onboarding: {
    done: false, // мастер первого запуска пройден или пропущен
    tourDone: false, // подсказки по интерфейсу показаны
  },
  ...
  migrations: {
    ...
    onboardingChecked: false, // решено, показывать ли мастер тем, кто уже работал
  },
